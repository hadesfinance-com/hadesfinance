...
// Hades Vault distributes fees equally amongst staked pools
// Have fun reading it. Hopefully it's bug-free. God bless.
contract HadesVault is OwnableUpgradeSafe {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of Hades
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accHadesPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws  tokens to a pool. Here's what happens:
        //   1. The pool's `accHadesPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. Hades to distribute per block.
        uint256 accHadesPerShare; // Accumulated Hades per share, times 1e12. See below.
    ...
    // Note contract owner is meant to be a governance contract allowing hade governance consensus
    function add(
        uint256 _allocPoint,
        IERC20 _token,
        bool _withUpdate,
    ...
    // Update the given pool's hades allocation point. Can only be called by the owner.
        // Note contract owner is meant to be a governance contract allowing hade governance consensus
    ...
    // View function to see pending hades on frontend.
    function pendinghade(uint256 _pid, address _user)
        external
        view
        returns (uint256)
    {
        ...
        uint256 acchadePerShare = pool.acchadePerShare;

        return user.amount.mul(acchadePerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    // ----
    // Function that adds pending rewards, called by the hade token.
    // ----
    uint256 private hadeBalance;
    function addPendingRewards(uint256 _) public {
        uint256 newRewards = hade.balanceOf(address(this)).sub(hadeBalance);

        if(newRewards > 0) {
            hadeBalance = hade.balanceOf(address(this)); // If there is no change the balance didn't change
    ...
    function updatePool(uint256 _pid) internal returns (uint256 hadeRewardWhole) {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        hadeRewardWhole = pendingRewards // Multiplies pending rewards by allocation point of this pool and then total allocation
        ...
        uint256 hadeRewardFee = hadeRewardWhole.mul(DEV_FEE).div(10000);
        uint256 hadeRewardToDistribute = hadeRewardWhole.sub(hadeRewardFee);

        pending_DEV_rewards = pending_DEV_rewards.add(hadeRewardFee);

        pool.acchadePerShare = pool.acchadePerShare.add(
            hadeRewardToDistribute.mul(1e12).div(tokenSupply)
        );

    }

    // Deposit  tokens to hadeVault for hade allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.acchadePerShare).div(1e12);
    ...
    function depositFor(address depositFor, uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.acchadePerShare).div(1e12); /// This is deposited for address
    ...
    // Withdraw  tokens from hadeVault.
    ...
    function _withdraw(uint256 _pid, uint256 _amount, address from, address to) internal {
        ...
        massUpdatePools();
        updateAndPayOutPending(_pid,  pool, user, from); // Update balances of from this is not withdrawal but claiming hade farmed
    ...
    function updateAndPayOutPending(uint256 _pid, PoolInfo storage pool, UserInfo storage user, address from) internal {
        ...
        uint256 pending = user
            .amount
            .mul(pool.acchadePerShare)
            .div(1e12)
            .sub(user.rewardDebt);

        if(pending > 0) {
            safehadeTransfer(from, pending);
        }
    ...
    // Safe hade transfer function, just in case if rounding error causes pool to not have enough hades.
    function safehadeTransfer(address _to, uint256 _amount) internal {
        if(_amount == 0) return;

        uint256 hadeBal = hade.balanceOf(address(this));
        if (_amount > hadeBal) {
            console.log("transfering out for to person:", _amount);
            console.log("Balance of this address is :", hadeBal);

            hade.transfer(_to, hadeBal);
            hadeBalance = hade.balanceOf(address(this));

        } else {
            hade.transfer(_to, _amount);
            hadeBalance = hade.balanceOf(address(this));
        ...
        if(pending_DEV_rewards > 0) {
            ...
            safehadeTransfer(devaddr, devSend);
        }
    ...
    // allowing hade governance token holders to do this functions.
